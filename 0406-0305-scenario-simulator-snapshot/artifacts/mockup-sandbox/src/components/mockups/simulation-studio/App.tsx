...
// Used by the participant view to broadcast its state to the Moderator
// Dashboard via the shared store.

const LIVE_TEAM_ID = "team-03";
...
function buildProgress(
  ...
  const blank: ProgressRecord["progress"] = {
    brief: "notStarted",
  ...
  const set = (over: Partial<ProgressRecord["progress"]>): ProgressRecord["progress"] => ({
    ...blank,
    ...over,
  });
  ...
  switch (screen) {
    ...
    default: {
      const _exhaustive: StepState = "notStarted" as StepState;
      void _exhaustive;
      return {
        currentStep: "Brief",
        currentStatus: "Joining session",
...
// ─── Moderator-to-participant message overlay ───────────────────────────────
// Listens for moderator notes (targeted at this team or "all") and access
// requests, and shows them as dismissible cards in the corner of the
// participant view.

function ModeratorMessages({ teamId }: { teamId: string }) {
  const notes = useStoreSnapshot(loadNotes);
  const accessRequests = useStoreSnapshot(loadAccessRequests);
  const [seenAt] = useState<number>(() => Date.now());
  ...
  for (const n of notes) {
    if (n.timestamp < seenAt) continue;
    if (n.recipient !== "all" && n.recipient !== teamId) continue;
    items.push({
      id: `note:${n.id}`,
      kind: "note",
      title: n.recipient === "all" ? "Moderator note · All Teams" : "Moderator note",
      body: n.message,
      timestamp: n.timestamp,
    });
  }
  for (const r of accessRequests) {
    if (r.timestamp < seenAt) continue;
    if (r.teamId !== teamId) continue;
    items.push({
      id: `access:${r.id}`,
      kind: "access",
      title: "Moderator requested access",
      body: "The moderator is asking to view or join your team workspace.",
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] w-[340px] flex flex-col gap-2 pointer-events-none">
      {visible.map((it) => (
        <div
          key={it.id}
          className={`pointer-events-auto rounded-xl border backdrop-blur-md shadow-2xl px-4 py-3 animate-in fade-in slide-in-from-bottom-2 duration-300 ${
            it.kind === "access"
            ...
            <div className="flex-1 min-w-0">
              <div
                className={`text-[10px] font-semibold uppercase tracking-[0.14em] mb-0.5 ${
                  it.kind === "access" ? "text-violet-200/90" : "text-yellow-200/90"
                }`}
              >
                {it.title}
              </div>
              <div className="text-white/85 text-[12.5px] leading-snug">{it.body}</div>
            </div>
            <button
              type="button"
              onClick={() =>
              ...
              aria-label="Dismiss"
              className="flex-shrink-0 w-6 h-6 inline-flex items-center justify-center rounded-md text-white/45 hover:text-white hover:bg-white/[0.06] transition-colors"
            >
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none">
                <path
...
export function App() {
  ...
  // ── Live progress + activity emission ──────────────────────────────────
  // Broadcast every meaningful state change to the shared store so that the
  // Moderator Dashboard reflects this team in real time.
  const startedAtRef = useRef<number>(Date.now());
  const stepStartedAtRef = useRef<number>(Date.now());
  const lastStepRef = useRef<CurrentStep | null>(null);
  const prevScreenRef = useRef<Screen | null>(null);
  const prevStakeholderRef = useRef<string | null>(null);
  const prevEvidenceRef = useRef<string | null>(null);

  useEffect(() => {
    const built = buildProgress(screen, selectedStakeholder, selectedEvidenceSource);
    if (lastStepRef.current !== built.currentStep) {
      stepStartedAtRef.current = Date.now();
      lastStepRef.current = built.currentStep;
    }
    upsertProgress({
      teamId: LIVE_TEAM_ID,
      ...
      currentStatus: built.currentStatus,
      latestActivity: built.latestActivity,
  ...
  useEffect(() => {
    ...
    const meta = { teamId: LIVE_TEAM_ID, teamName } as const;

    if (prev === null && screen === "entry") {
      appendActivity({ ...meta, text: `${teamName} joined the session`, tone: "info" });
      return;
    }

    if (screen === "intro" && prev === "investigate" && selectedStakeholder) {
      const s = stakeholders.find((x) => x.id === selectedStakeholder);
      if (s) appendActivity({ ...meta, text: `${teamName} selected ${s.name}`, tone: "info" });
    }
    ...
    if (qIdx >= 0) {
      appendActivity({
        ...meta,
        text: `${teamName} reached Question ${qIdx + 1} of ${QUESTION_SCREENS.length}`,
        tone: "info",
      });
    }

    if (screen === "evidence_reveal" && selectedEvidenceSource) {
      const e = evidenceSources.find((x) => x.id === selectedEvidenceSource);
      if (e) appendActivity({ ...meta, text: `${teamName} selected ${e.title}`, tone: "info" });
    }

    if (screen === "problem") {
      appendActivity({ ...meta, text: `${teamName} started the problem statement`, tone: "info" });
    }

    if (screen === "confirm") {
      appendActivity({
        ...meta,
        text: `${teamName} submitted problem statement`,
        tone: "info",
      });
    }
  }, [screen, selectedStakeholder, selectedEvidenceSource]);

  useEffect(() => {
    if (selectedStakeholder && selectedStakeholder !== prevStakeholderRef.current) {
      prevStakeholderRef.current = selectedStakeholder;
      const s = stakeholders.find((x) => x.id === selectedStakeholder);
      if (s) {
        appendActivity({
          teamId: LIVE_TEAM_ID,
          teamName: session.team,
          text: `${session.team} chose ${s.name} as their stakeholder`,
          tone: "info",
        });
      }
    }
  ...
  useEffect(() => {
    if (selectedEvidenceSource && selectedEvidenceSource !== prevEvidenceRef.current) {
      prevEvidenceRef.current = selectedEvidenceSource;
      const e = evidenceSources.find((x) => x.id === selectedEvidenceSource);
      if (e) {
        appendActivity({
          teamId: LIVE_TEAM_ID,
          teamName: session.team,
          text: `${session.team} chose ${e.title} as evidence`,
          tone: "info",
        });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#100f24] via-[#100a23] to-[#110322] text-white relative overflow-x-hidden font-sans">
      ...
      <ModeratorMessages teamId={LIVE_TEAM_ID} />
    </div>
  );
}
