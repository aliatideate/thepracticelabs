...
export default function SimulationStudioCopyW8YFiH6Z() {
  ...
  const prependActivity = (text: string, tone?: "info" | "attention" | "moderator") => {
    const t = activity[0]?.time ?? "00:00";
    setActivity((prev) => [{ time: t, text, tone }, ...prev]);
  };

  const handleSendNote = (recipientLabel: string, _message: string) => {
    show(`Note sent to ${recipientLabel}.`);
    prependActivity(`Moderator note sent to ${recipientLabel}.`, "moderator");
  };

  const handleSendNudge = (teamName: string) => {
    show(`Nudge sent to ${teamName}.`);
    prependActivity(`Moderator nudge sent to ${teamName}.`, "moderator");
  };

  const requestAccessConfirmed = (teamId: string) => {
    const team = teams.find((t) => t.id === teamId);
    if (!team) return;
    setAccessRequested((p) => ({ ...p, [teamId]: true }));
    show(`Access request sent to ${team.name}.`);
    prependActivity(`Access requested for ${team.name}`, "moderator");
  ...
  const markForDebrief = (teamId: string) => {
    ...
    if (wasMarked) {
      ...
      prependActivity(`${team.name} unmarked for debrief`, "moderator");
    } else {
      show(`${team.name} marked for debrief.`);
      prependActivity(`${team.name} marked for debrief`, "moderator");
    }
  };
  ...
  return (
    <div className="min-h-screen text-white bg-gradient-to-br from-[#100f24] via-[#100a23] to-[#110322]">
      ...
      <div className="relative">
        ...
        <footer className="mt-10 border-t border-white/[0.06] bg-[#0c0a1f]/60 backdrop-blur-md">
          <div className="max-w-[1400px] mx-auto px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <img src={assets.scenarioSimulatorLogo} alt="Scenario Simulator" className="h-5 w-auto opacity-80" />
              <span className="text-[11px] text-white/45">
                © {new Date().getFullYear()} Ideate Internal Tools · Scenario Simulator
              </span>
            </div>
            <div className="flex items-center gap-4 text-[11px] text-white/45">
              <a href="#" className="hover:text-white/75 transition-colors">Help</a>
              <a href="#" className="hover:text-white/75 transition-colors">Facilitator guide</a>
              <a href="#" className="hover:text-white/75 transition-colors">Send feedback</a>
